...
// Nick: "192.168.2.122"
// const char * udpAddress = "192.168.2.255"; // Jetson
const char * udpAddress = "192.168.2.102"; // Adrian
...
void setup() {
  ...
  Serial.print("Connected - Sending packet");
  sleep(2);
  //Send a packet
  while (true)
  {
    ...
    if(packetSize >= sizeof(float)){
      float empty_array[1]; 
      udp.read((char*)empty_array, sizeof(empty_array)); 
      udp.flush();
      break;
    }
  }

  Serial.print("Packet Sent and Received");
