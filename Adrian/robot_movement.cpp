...
// Home
//const char * networkName = "Fios-IKDHK";
//const char * networkPswd = "ape6816pyro3572toy";
// School
...
// Home
//const char * udpAddress = "192.168.1.249";
...
void loop() {
  ...
  while (true) {
    ...
    if(packetSize >= sizeof(float))
    {
      ...
      udp.flush();
      Serial.printf("dB_sum received value is %f\n", my_array[0]);
      Serial.printf("python_max received value is %f\n", my_array[1]);
      dB_sum = my_array[0];
      python_max = my_array[1];
    }
    ...
    if (dB_sum >= -45 && dB_sum != 0) {
      Serial.printf("TRUE and dB_sum is %f and time is %f\n", dB_sum, t);
      //target_omega = 0.5*sin(2*t) - python_max;
      target_omega = 0.0;
      target_v = 0.0;
    } else {
      //target_omega = 0.5*sin(2*t);
      target_omega = 0.0;
      target_v = 0.0;
    }
